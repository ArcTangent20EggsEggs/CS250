...
{
	cout << "Please enter the maximum capacity ";
	int temp;
	cin >> temp;
	m_airport.SetMaxCapacity(temp);
...
bool AirTravelSimulator::LineUp()
{
	...
	int temp =  rand() % 1000;
	Traveller* ptr = m_pplManager.GetTraveller(temp);
	m_airport.LineUp(ptr);
	m_ptrTravellers.PushFront(ptr);
	ptr->state = LINEUP;
...
/**
    ...
    @return <bool>
*/
bool AirTravelSimulator::Boarding()
{
