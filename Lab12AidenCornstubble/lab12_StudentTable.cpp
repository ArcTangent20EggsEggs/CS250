...
{
	int ind;
	ind = HashFunction(studentId);
	m_students[ind].key = studentId;
	m_students[ind].value = name;
}
...
{
	int ind = key % TABLE_SIZE;
	int originalInd = ind;
	int quadAdd = 1;
	int dubHashAdd = 0;
	while (m_students[ind].key != -1)
	{
		if (m_collisionMethod == LINEAR)
		{
			ind = LinearProbe(ind);
		...
		{
			ind = QuadraticProbe(originalInd, quadAdd);
			ind %= TABLE_SIZE;
		}
		if (m_collisionMethod == DOUBLE)
		{
			dubHashAdd += HashFunction2(originalInd);
			ind = (originalInd + dubHashAdd) % TABLE_SIZE;
		}
	}
	return ind;
}
