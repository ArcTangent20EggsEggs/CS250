...
public:
    ...
    */
    void Delete( const TK& key )
    {
		...
		else
		{
			...
			Node<TK, TD> *replacement = node;
			Node<TK, TD> *replacementParent = node;
			...
			if (replacement != node->ptrRight)
			{
				replacementParent->ptrLeft = replacement->ptrRight;
				replacement->ptrRight = node->ptrRight;
			}

			if (node == m_ptrRoot)
			{
				m_ptrRoot = replacement;
			}

			else if (leftNode)
			{
				node = parent->ptrLeft;
				parent->ptrLeft = replacement;
			}
			else
			{
				node = parent->ptrRight;
				parent->ptrRight = replacement;
			}
			replacement->ptrLeft = node->ptrLeft;
		}
    ...
    */
    bool Contains( const TK& key )
    {
		...
		while (temp->key != key)
		{
			if (key > temp->key && temp->ptrRight != nullptr)
			{
				temp = temp->ptrRight;
			}
			else if (key <= temp->key && temp->ptrLeft != nullptr)
			{
				temp = temp->ptrLeft;
			}
			else
...
private:
    ...
    */
    Node<TK, TD>* FindParentOfNode( const TK& key )
    {
		...
		Node<TK, TD> *temp = m_ptrRoot;
		while (temp->ptrRight->key != key || temp->ptrLeft->key != key)
		{
			if (key > temp->key && temp->ptrRight != nullptr)
			{
				temp = temp->ptrRight;
    ...
    */
    int GetHeight( Node<TK, TD>* ptrCurrent )
    {
		...
		int lH = 0;
		int rH = 0;
		while (tempL->ptrLeft != nullptr)
		{
			tempL = tempL->ptrLeft;
			lH++;
		}
		while (tempR->ptrRight != nullptr)
		{
			tempR = tempR->ptrRight;
			rH++;
		}
		if (lH >= rH)
			return lH + 1;
